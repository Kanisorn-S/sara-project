...
First, the integral term was disabled ($K_I = 0$), and the proportional gain $K_P$ was gradually increased until the system output began to oscillate with constant frequency. This gain value is known as the \textit{ultimate gain} $K_u$, and the oscillation period is the \textit{ultimate period} $T_u$.
...
\begin{align}
    ...
    K_I &= \frac{0.54 K_P}{T_u} \label{eq:zn-ki}
\end{align}

Substituting the measured values of $K_u$ and $P_u$ gives:
\begin{align*}
    K_P &= 0.45(2.4) = 1.08 \\
    K_I &= \frac{0.54(1.08)}{12.5} = 0.104
\end{align*}
...
To verify the effectiveness of the PI controller, a series of tests were conducted where the target temperature was set to various values. The system's response was monitored to assess how quickly and accurately it could reach and maintain the desired temperature. 

The experiment was set up as follows:
\begin{itemize}
    \item The ambient temperature was approximately \SI{27.5}{\celsius}.
    \item The target temperature was set to \SI{27.8}{\celsius}, \SI{28.0}{\celsius}, and \SI{28.3}{\celsius} in separate trials.
    \item The temperature of the system was measured every 5 seconds using a reliable digital thermometer placed in close proximity to the thermistor.
    \item A timer was used to record the time taken to reach the target temperature and the stability of the temperature once reached.
\end{itemize}
